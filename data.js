...
/* -------------------------------------------------------------------------
   ...
   ※スプレッドシートで管理することもできます（Googleフォームは使いません）。
     ...
     ・役職：選手には空欄のままでOK。選手以外の人（マネージャー、Web管理者など）
       ...
       そのまま表示されます
     ・保有資格：指導者ライセンス・審判資格など、あれば自由に入力（例：「Cライセンス」「3級審判」）。空欄でもOK。
       入力すると出身校の下に小さいバッジで表示されます。複数持っている場合は、
       セルの中でAlt+Enter（Windowsの場合はCtrl+Enter）で改行して1行に1つずつ書くと、
       バッジがその数だけ分かれて表示されます
     ・写真ファイル名：images/ は省略してOK。"kaneko.jpg" と入力するだけで
       自動的に "images/kaneko.webp" として扱われます（空欄でもOK）
     ※列名は完全一致でなくてOK。「名前」「学年」「出身」「役職」「資格」「写真」
